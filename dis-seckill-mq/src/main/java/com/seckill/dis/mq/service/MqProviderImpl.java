...
public class MqProviderImpl implements MqProviderApi, RabbitTemplate.ConfirmCallback {
    ...
    @Override
    public void sendReplenishMessage(ReplenishMessage message) {
        logger.info("MQ send replenish message: goodsId=" + message.getGoodsId() + ", quantity=" + message.getQuantity());
        CorrelationData corrData = new CorrelationData(UUID.randomUUID().toString());
        rabbitTemplate.convertAndSend(MQConfig.REPLENISH_QUEUE, message, corrData);
    }
